...
2. **Leggi `goal.md`**. Se contiene una direttiva (testo non commentato, fuori dai blocchi `<!-- ... -->`), quello è il tuo focus prioritario per questa sessione. Se è vuoto o contiene solo commenti, esplora liberamente seguendo `program.md`.
3. Esegui la sezione **Setup** di `program.md`:
...
4. Riepiloga in 1-2 righe: tag scelto + goal letto da `goal.md` (oppure "esplorazione libera").
5. Entra nell'**Experiment loop** (sezione "The experiment loop" di `program.md`).
...
- Se hai un goal in `goal.md`, ogni nuova idea deve essere coerente con esso (a meno che tu non sia esplicitamente in stallo dopo molte run).
